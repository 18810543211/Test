...
public class MyNestedScrollParent extends LinearLayout implements NestedScrollingParent {
    ...
    protected void onFinishInflate() {
        shrinkView = (LinearLayout) getChildAt(0);
        fixedView = (LinearLayout) getChildAt(1);
        myNestedScrollChild = (MyNestedScrollChild) getChildAt(2);
        shrinkView.getViewTreeObserver().addOnGlobalLayoutListener(() -> {
            if (imgHeight <= 0) {
                imgHeight = shrinkView.getMeasuredHeight();
            }
        });
        fixedView.getViewTreeObserver().addOnGlobalLayoutListener(() -> {
            if (tvHeight <= 0) {
                tvHeight = fixedView.getMeasuredHeight();
            }
        });
    }
